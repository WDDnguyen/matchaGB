...
void set_nintendo_logo_data(cartridge *cartridge_p){
    ...
    for (int i = 0; i < 48; i++){
        cartridge_p->cartridge_memory[0x104 + i] = nintendo_logo_data[i];
        //printf("Cartridge[0x%4X] IN CARTRIDGE 0x%02X \n", (0x104 + i), cartridge_p->cartridge_memory[0x104 + i]);
    }
}
