...
static int execute_opcode(cpu *cpu_p, byte opcode){
    ...
    switch(opcode){
        ...
        case 0x06: load_immediate_8_bit(cpu_p, &cpu_p->BC.high); return 8;
        case 0x0E: load_immediate_8_bit(cpu_p, &cpu_p->BC.low); return 8;
        case 0x16: load_immediate_8_bit(cpu_p, &cpu_p->DE.high); return 8;
        case 0x1E: load_immediate_8_bit(cpu_p, &cpu_p->DE.low); return 8;
        case 0x26: load_immediate_8_bit(cpu_p, &cpu_p->HL.high); return 8;
        case 0x2E: load_immediate_8_bit(cpu_p, &cpu_p->HL.low); return 8;
        ...
        case 0x0A: load_8_bit(&cpu_p->AF.high, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->BC))); return 8; 
        case 0x1A: load_8_bit(&cpu_p->AF.high, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->DE))); return 8;
        case 0xFA: load_8_bit(&cpu_p->AF.high, read_memory(cpu_p->memory_p, get_immediate_16_bit(cpu_p))); return 16;
        case 0x3E: load_immediate_8_bit(cpu_p, &cpu_p->AF.high); return 16;
        
        // LD r1, r2 when r1 == B
        case 0x47: load_8_bit(&cpu_p->BC.high, cpu_p->AF.high); return 4;
        ...
        // LD r1, r2 when r1 == C
        case 0x4F: load_8_bit(&cpu_p->BC.low, cpu_p->AF.high); return 4;
        ...
        // LD r1, r2 when r1 == D
        case 0x57: load_8_bit(&cpu_p->DE.high, cpu_p->AF.high); return 4;
        ...
        // LD r1, r2 when r1 == E
        case 0x5F: load_8_bit(&cpu_p->DE.low, cpu_p->AF.high); return 4;
        ...
        // LD r1, r2 when r1 == H
        case 0x67: load_8_bit(&cpu_p->HL.high, cpu_p->AF.high); return 4;
        ...
        // LD r1, r2 when r1 == L
        case 0x6F: load_8_bit(&cpu_p->HL.low, cpu_p->AF.high); return 4;
        ...
        // LD r1, r2 when r2 == (HL)
        case 0x7E: load_8_bit(&cpu_p->AF.high, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL))); return 8;
        case 0x46: load_8_bit(&cpu_p->BC.high, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL))); return 8;
        case 0x4E: load_8_bit(&cpu_p->BC.low, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL))); return 8;
        case 0x56: load_8_bit(&cpu_p->DE.high, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL))); return 8;
        case 0x5E: load_8_bit(&cpu_p->DE.low, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL))); return 8;
        case 0x66: load_8_bit(&cpu_p->HL.high, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL))); return 8;
        case 0x6E: load_8_bit(&cpu_p->HL.low, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL))); return 8; 
        
        // LD r1, r2 when r1 == (HL) 
        case 0x70: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL), cpu_p->BC.high); return 8;
        case 0x71: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL), cpu_p->BC.low); return 8;
        case 0x72: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL), cpu_p->DE.high); return 8;
        case 0x73: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL), cpu_p->DE.low); return 8;
        case 0x74: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL), cpu_p->HL.high); return 8;
        case 0x75: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL), cpu_p->HL.low); return 8;
        ...
        case 0x02: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->BC), cpu_p->AF.high); return 8;
        case 0x12: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->DE), cpu_p->AF.high); return 8;
        case 0x77: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL), cpu_p->AF.high); return 8;
        case 0xEA: write_memory(cpu_p->memory_p, get_immediate_16_bit(cpu_p), cpu_p->AF.high); return 16;
        
        // LD A, (C)
        case 0xF2: load_8_bit(&cpu_p->AF.high, read_memory(cpu_p->memory_p, 0xFF00 + cpu_p->BC.low)); return 8;
        // LD (C), A
        case 0xE2: write_memory(cpu_p->memory_p, 0xFF00 + cpu_p->BC.low, cpu_p->AF.high); return 8;
        
        // LD A, (HLD)
        case 0x3A: load_8_bit(&cpu_p->AF.high, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL))); decrement(&cpu_p->HL); return 8;
        // LD (HLD), A
        case 0x32: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL), cpu_p->AF.high); decrement(&cpu_p->HL); return 8; 
        
        // LD A, (HLI)
        case 0x2A: load_8_bit(&cpu_p->AF.high, read_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL))); increment(&cpu_p->HL); return 8;
        // LD, (HLI), A
        case 0x22: write_memory(cpu_p->memory_p, get_registers_word(&cpu_p->HL), cpu_p->AF.high); increment(&cpu_p->HL); return 8;

        // LDH (n), A
        case 0xE0: write_memory(cpu_p->memory_p, 0xFF00 + cpu_p->PC, cpu_p->AF.high); cpu_p->PC++; return 12;
        // LDH A, (n)
        case 0xF0: load_8_bit(&cpu_p->AF.high, read_memory(cpu_p->memory_p, 0xFF00 + cpu_p->PC)); cpu_p->PC++; return 12;
        ...
        case 0x08: 
            ...
            write_memory(cpu_p->memory_p, address, cpu_p->SP.low);
            address++;
            write_memory(cpu_p->memory_p, address, cpu_p->SP.high);
            return 20; 
    }
...
word get_registers_word(cpu_register *register_p){
    return ((register_p->high << 8) | (register_p->low));
}

static void set_registers_word(cpu_register *register_p, word data){
    register_p->high = (data & 0xFF00) >> 8;
    register_p->low = (data * 0x00FF);
}
...
static void load_hl(cpu *cpu_p, cpu_register *AF_p, cpu_register *HL_p){
    ...
    AF_p->low = CLEAR_BIT(AF_p->low, ZERO_FLAG);
    AF_p->low = CLEAR_BIT(AF_p->low, SUBTRACT_FLAG);
    ...
    if (overflow > 0xFFFF){
        AF_p->low = SET_BIT(AF_p->low, CARRY_FLAG);
    } else {
        AF_p->low = CLEAR_BIT(AF_p->low, CARRY_FLAG);
    }

    overflow = (get_registers_word(&cpu_p->SP) & 0xF) + (n & 0xF);
    if (overflow > 0xF){
        AF_p->low = SET_BIT(AF_p->low, HALF_CARRY_FLAG);
    } else {
        AF_p->low = CLEAR_BIT(AF_p->low, CARRY_FLAG);
    }
} 
...
static void increment(cpu_register *register_p){
    ...
    value++;
    register_p->high = (value & 0xFF00) >> 8;
    register_p->low = (value & 0x00FF);
}
